...
# load data
data_imp <- read.csv("data_pop_2_imp.csv")%>%
  filter(vote_pop_2 != 4 & vote_pop_2 != 0)%>% # exclude abstainers and non-populist voters
...
# nominal variables
fac_vars <- c("cntry", "blgetmg", "uemp3m", "vote_pop_2", "gndr", "pop_choice_2",
...
# split data
set.seed(098)
split <- initial_split(data_imp, strata = vote_pop_2)
train <- training(split)
test <- testing(split)


# build recipe
rf_rec <- recipe(vote_pop_2~., data = train)%>%
  update_role(idno, new_role = "ID") %>%
  themis::step_upsample(vote_pop_2) # upsample data
...
set.seed(876)
folds <- vfold_cv(train, v=5, strata = vote_pop_2)
...
# best values
tune_res %>% select_best(metric = "accuracy") #mtry = 14 , min_n 8
...
best_auc <- select_best(regular_res, "roc_auc") # mtry = 12, min_n = 6
...
# write to disk
write_rds(final_wf, "~/Uni/Master/Masterarbeit/Results/Final Results/RF6_wf.rds")
...
# write to disk
write_rds(final_result, "~/Uni/Master/Masterarbeit/Results/Final Results/RF6_result.rds")
...
true <- final_result %>%
  collect_predictions() %>%
  dplyr::select(vote_pop_2)

# generate confusion matrix
conf <- caret::confusionMatrix(pred$.pred_class, true$vote_pop_2)

# extract performance measures
class <- as.data.frame(conf$byClass)

# model performance table
performance_RF6 <- class%>%
