...
data_imp <- read.csv("data_pop_1_imp.csv")%>%
  ...
  filter(cntry != "IS" & cntry != "LV", cntry != "RS")%>%
  dplyr::select(-c(X))
...
# best values
tune_res %>% select_best(metric = "accuracy") #mtry = 15 , min_n 28
...
best_auc <- select_best(regular_res, "roc_auc") # mtry = 16, min_n = 27
...
# write to disk
write_rds(final_wf, "~/Uni/Master/Masterarbeit/Results/Final Results/RF5_wf.rds")
...
# write to disk
write_rds(final_result, "~/Uni/Master/Masterarbeit/Results/Final Results/RF5_result.rds")
